...
def _load_strategy(name: str, **params):
    ...
    mod_map = {"ma_cross": ("example_ma_cross", "MACrossStrategy")}
...
def run_backtest(symbol: str, strategy_name: str, lookback_days: int = 730,
                  validation_days: int = 120, **strategy_params) -> tuple[BacktestResult, BacktestResult]:
    ...
    def _run(segment: pd.DataFrame) -> BacktestResult:
        ...
        eq = np.array(equity_curve) if equity_curve else np.array([DEFAULT_CONFIG.risk.starting_capital])
        ...
        return BacktestResult(
            ...
            total_return_pct=(eq[-1] / DEFAULT_CONFIG.risk.starting_capital - 1) * 100 if len(eq) else 0.0,
